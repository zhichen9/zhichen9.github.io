...
#gallery-nav{
  ...
  text-align:center; padding:10px; border-bottom:1px solid #ddd; margin-bottom:12px;
}
#gallery-nav a{
...
/* 章节头部 + 折叠/展开 */
.country-header{ margin-bottom:6px; }
.country-header h3{
  color:var(--theme1); font-size:20px; margin:0 0 6px; font-weight:700;
...
.gallery a{
  position:relative; display:inline-block; break-inside:avoid; /* 防止跨列断裂 */
}
.gallery img{
  width:100%; height:auto; border-radius:8px; margin-bottom:15px;
  opacity:0; transform:translateY(15px);
  transition:all .8s ease; will-change:transform,opacity;
}
